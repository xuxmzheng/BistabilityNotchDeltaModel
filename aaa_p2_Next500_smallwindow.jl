...
for Dx in d_vals
    ...
    if length(roots) == 1
        ...
        if nr < 0.5
            push!(b2_N, Point2f(Dx, nr * N0))
            push!(b2_D, Point2f(Dx, dr * D0))
        else
            push!(b1_N, Point2f(Dx, nr * N0))
            push!(b1_D, Point2f(Dx, dr * D0))
        end
...
# --- 3. Cleaning (only break if molecule jumps too much) ---
function clean_branch(pts)
    ...
    for i in 2:length(pts)
        if abs(pts[i][2] - pts[i-1][2]) > 2000.0
...
b1_N_c = clean_branch(b1_N)
b1_D_c = clean_branch(b1_D)
b2_N_c = clean_branch(b2_N)
b2_D_c = clean_branch(b2_D)
bm_N_c = clean_branch(b_mid_N)
...
display(fig)
save("bifurcation_p2_Next500_smallwindow.png", fig)